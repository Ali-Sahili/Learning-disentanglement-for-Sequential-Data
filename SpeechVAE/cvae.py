...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        output = input.view(-1, 1, self.frames, self.F)
...
class Decoder(nn.Module):
    def __init__(self, z_dim, dropout=0.2, relu=0.1, n_filters = 80):
        ...
        Cov2: filters = 128, filter size = 3 * 1, stride = (2,1), tanh(), BatchNorm
        Conv1: filters = 64, filter size = 1 * F, stride = (1,1), tanh(), BatchNorm
        1*(128*1) --> 1*(512*1) --> 256*(4*1) --> 128*(9*1) --> 64*(frames*1) --> 1*(frames*80)
        '''
        self.frames = 19
    ...
    def forward(self, input):
        ...
        output = output.view(-1, self.frames, self.F)
        return output
