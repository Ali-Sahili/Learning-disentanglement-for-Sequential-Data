...
class Encoder(nn.Module):
    def __init__(self, z_dim=64, log_var_=None, dropout=0.2, relu=0.1, n_filters=80):
    ...
    def forward(self, input):
        output = input.contiguous().view(self.frames, -1, self.F)
...
class Decoder(nn.Module):
    def __init__(self, z_dim=64, dropout=0.2, relu=0.1, n_filters=80):
    ...
    def forward(self, input):
        ...
        #print(inputs.size())
        inputs = inputs.repeat(1, self.frames, 1)
        #print(inputs.size())
        inputs = inputs.permute(1,0,2)
        #print(inputs.size())
        out, _ = self.lstm(inputs)
        #print(out.size())
        out = out.view(-1, self.frames, 512)
        out = self.fc(out)
        output = out.view(-1, self.frames, self.F)
        return output
...
class VAE(nn.Module):
    def __init__(self, z_dim, log_var_=None, dropout=0.2, relu=0.1, n_filters=80):
