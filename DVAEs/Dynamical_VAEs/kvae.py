...
class KVAE(nn.Module):
    ...
    def forward(self, x, compute_loss=True):
        ...
        # calculate loss
        loss_tot, loss_vae, loss_lgssm = self.get_loss(x, y, u, a, a_mean, a_logvar, 
        ...
        self.a = a.permute(1,-1,0).squeeze()
        
        #self.u = u.permute(1,-1,0).squeeze()
        #self.a_mean = a_mean.permute(1,-1,0).squeeze()
        #self.a_logvar = a_logvar.permute(1,-1,0).squeeze()
        #self.mu_smooth = mu_smooth.permute(1,-1,0).squeeze()
        #self.Sigma_smooth = Sigma_smooth.permute(1,-1,0).squeeze()
        #self.A_mix = A_mix.permute(1,-1,0).squeeze()
        #self.B_mix = B_mix.permute(1,-1,0).squeeze()
        #self.C_mix = C_mix.permute(1,-1,0).squeeze()

        return self.y, self.a, loss_tot, loss_vae, loss_lgssm
